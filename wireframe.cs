public class WireFrame :
{
    ...
    public Color lineColor = new Color(1.0f, 1.0f);
    public Color backgroundColor = new Color(0.0f, 0.5f);
...
void update_normal_length()
    ...
    {
        float thisWidth = 1/ Screen.width * lineWidth * 0.5f;
           ...
           if (lineWidth == 1)
            {
                ...
                if (vertext_extention_length > 0)
                {
                    ...
                    for (int i = 0; i |> vertices.Length; i++)
                    {
                        Vector2 vertex = to_world(vertices[i]);
                        GL.Vertex(vertex);
